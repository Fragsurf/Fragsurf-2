...
namespace RealtimeCSG
{
    internal sealed class EditModeMeshEdit : ScriptableObject, IEditMode
    {
        ...
        private void ExtrudeSurface(Camera camera, bool drag)
        {
            ...
            if (polygon == null
                || polygon.EdgeIndices == null
                || polygon.EdgeIndices.Length == 0)
            {
                return;
            }
        ...
        bool ShouldDrawAnyPoints(Camera camera)
        {
            return true;
        }
        ...
        private void OnPaint(SceneView sceneView)
        {
...

            {
                ...
                if (ShouldDrawAnyPoints(camera) && (CSGSettings.SelectionVertex || CSGSettings.SelectionSurface))
                {
                    ...
                    for (var t = 0; t < _brushSelection.Brushes.Length; t++)
                    {
                        var meshState = _brushSelection.States[t];
                        var modelTransform = _brushSelection.ModelTransforms[t];
                        var camState = _brushSelection.States[t].GetCameraState(camera, false);
                        ...
                        if (CSGSettings.SelectionVertex && meshState.WorldPoints != null)
                        {
                            pointMeshManager.DrawPoints(camera, meshState.WorldPoints, camState.WorldPointSizes, camState.WorldPointColors);
                        }

                        if (CSGSettings.SelectionSurface && meshState.PolygonCenterPoints != null)
                        {
                            pointMeshManager.DrawPoints(camera, meshState.PolygonCenterPoints, camState.PolygonCenterPointSizes, meshState.PolygonCenterColors);
                        }
                    }
                    ...
                    case EventType.KeyDown:
                        {
                            ...
                            if(_hoverOnPointIndex != -1 
                                && _hoverOnTarget != -1
                                && Keys.SetPivotKey.IsKeyPressed())
                            {
                                Event.current.Use();
                                var brush = _brushSelection.States[_hoverOnTarget];
                                var point = brush.WorldPoints[_hoverOnPointIndex];
                                BrushUtility.SetPivot(_brushSelection.Brushes[_hoverOnTarget], point);
                                Debug.Log("Set pivot: " + point);
                                break;
                            }
                            else break;
                        }
