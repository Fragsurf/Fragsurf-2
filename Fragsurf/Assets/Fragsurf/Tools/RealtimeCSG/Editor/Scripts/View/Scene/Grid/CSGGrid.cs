...
namespace RealtimeCSG
{
    ...
    {
        static Material _gridMaterial = null;
        static int gridSizeID = -1;
        static int gridSpacingID = -1;
        static int gridColorID = -1;
        static int gridCenterColorID = -1;
        static int gridLineThicknessID = -1;
        static int gridCenterLineThicknessID = -1;
        static int gridSubdivisionTransparencyID = -1;
        ...
        static Material GridMaterial
        {
            get
            {
                if (!_gridMaterial)
                {
                    ...
                    gridLineThicknessID = Shader.PropertyToID("_GridLineThickness");
                    gridCenterLineThicknessID = Shader.PropertyToID("_CenterLineThickness");
                    gridSubdivisionTransparencyID = Shader.PropertyToID("_SubdivisionTransparency");
                    ...
                    _gridMaterial.hideFlags = HideFlags.HideAndDontSave | HideFlags.DontUnloadUnusedAsset;

                    GridLineThickness = 1.0f;
                    GridCenterLineThickness = 1.0f;
                    GridSubdivisionTransparency = 0.7f;
                }
                return _gridMaterial;
            }
        }

        static float GridLineThickness
        {
            set => _gridMaterial?.SetFloat(gridLineThicknessID, value * EditorGUIUtility.pixelsPerPoint);
        }

        static float GridCenterLineThickness
        {
            set => _gridMaterial?.SetFloat(gridCenterLineThicknessID, value * EditorGUIUtility.pixelsPerPoint);
        }

        static float GridSubdivisionTransparency
        {
            set => _gridMaterial?.SetFloat(gridSubdivisionTransparencyID, value);
        }

        static Color GridCenterColor
        {
            set => _gridMaterial?.SetColor(gridCenterColorID, value);
        }

        static Color GridColor
        {
            set => _gridMaterial?.SetColor(gridColorID, value);
        }

        static float GridSize
        {
            set => _gridMaterial?.SetFloat(gridSizeID, value);
        }

        static Vector4 GridSpacing
        {
            set => _gridMaterial?.SetVector(gridSpacingID, value);
        }

        static Mesh _gridMesh;
        ...
        static void DrawGrid(SceneView sceneView, Vector3 cameraPosition, Vector3 gridCenter, Quaternion gridRotation, GridAxis axis, float alpha, GridMode gridMode)
        {
            if (alpha <= 0.03f
                || !GridMaterial
                || !GridMesh)
            ...
            if (gridMode == GridMode.Ortho)
            {
                ...
                for (int i = 0; i < 3; i++)
                {
                    if (!FastApproximately(normal[i], 0, .01f))
                    {
                        camPos[i] = 1000;
                    }
                }
                if(sceneView.camera.nearClipPlane > 1f)
                {
                    sceneView.cameraSettings.nearClip = 1f;
                    sceneView.camera.nearClipPlane = 1f;
                }
                if(sceneView.camera.farClipPlane < 2000)
                {
                    sceneView.cameraSettings.farClip = 2000;
                    sceneView.camera.farClipPlane = 2000;
                }
            ...
            var snapX = CSGSettings.SnapVector.x;
            var snapY = CSGSettings.SnapVector.y;
            var snapZ = CSGSettings.SnapVector.z;

            // calculate a point on the camera at the same distance as the camera is to the grid, in world-space
            var forward = sceneView.camera.cameraToWorldMatrix.MultiplyVector(MathConstants.forwardVector3);
            ...
            var sideways = sceneView.camera.cameraToWorldMatrix.MultiplyVector(MathConstants.leftVector3);
            var screenPointCenter = sceneView.camera.WorldToScreenPoint(projectedCenter);
            var screenPointAxisX = sceneView.camera.WorldToScreenPoint(projectedCenter + (sideways * snapX));
            var screenPointAxisY = sceneView.camera.WorldToScreenPoint(projectedCenter + (sideways * snapY));
            var screenPointAxisZ = sceneView.camera.WorldToScreenPoint(projectedCenter + (sideways * snapZ));
            ...
            var screenPixelSize = axis switch
            {
                GridAxis.AxisYZ => Mathf.Min(pixelSizeY, pixelSizeZ),
                GridAxis.AxisXY => Mathf.Min(pixelSizeX, pixelSizeY),
                GridAxis.AxisXZ => Mathf.Min(pixelSizeX, pixelSizeZ),
                _ => Mathf.Min(pixelSizeX, pixelSizeY)
            };

            const float minPixelSize = 64f;
            var gridLevelPow = (minPixelSize / screenPixelSize);

            GridSize = Mathf.Max(0, gridLevelPow);
            GridSpacing = CSGSettings.SnapVector;

            var gridCenterColor = ColorSettings.gridColorW;
            var gridColor = ColorSettings.gridColor1W;

            switch (gridMode)
            {
                case GridMode.WorkPlane:
                    gridCenterColor = Color.white;
                    gridColor = Color.white;
                    break;
                case GridMode.Regular:
                case GridMode.Ortho:
                    gridCenterColor = new Color(1.0f, 0.6f, 0f, .9f);
                    break;
            }

            gridColor.a = alpha;
            gridCenterColor.a = alpha;

            GridCenterColor = gridCenterColor;
            GridColor = gridColor;
        ...
        static void OnRender(SceneView sceneView)
        {
            UpdateGridOrientation(sceneView.camera);

            if (gridOrientation.gridOrtho)
            {
                if (gridOrientation.gridOrthoXVisible)
                {
                    DrawGrid(sceneView, gridOrientation.gridCameraPosition, gridOrientation.gridCenter, gridOrientation.gridOrthoXRotation, GridAxis.AxisYZ, gridOrientation.gridOrthoXAlpha, GridMode.Ortho);
                }

                if (gridOrientation.gridOrthoYVisible)
                {
                    DrawGrid(sceneView, gridOrientation.gridCameraPosition, gridOrientation.gridCenter, gridOrientation.gridOrthoYRotation, GridAxis.AxisXZ, gridOrientation.gridOrthoYAlpha, GridMode.Ortho);
                }

                if (gridOrientation.gridOrthoZVisible)
                {
                    DrawGrid(sceneView, gridOrientation.gridCameraPosition, gridOrientation.gridCenter, gridOrientation.gridOrthoZRotation, GridAxis.AxisXY, gridOrientation.gridOrthoZAlpha, GridMode.Ortho);
                }
            }
            else
            {
                ...
                if (ForceGrid &&
                    !((forward - work_forward).sqrMagnitude < 0.001f ||
                      (forward + work_forward).sqrMagnitude < 0.001f))
                {
                    DrawGrid(sceneView, gridOrientation.gridCameraPosition,
                             gridOrientation.gridWorkCenter, gridOrientation.gridWorkRotation,
                             GridAxis.AxisXZ, .7f, GridMode.WorkPlane);
                ...
                {
                    DrawGrid(sceneView, gridOrientation.gridCameraPosition, gridOrientation.gridWorkCenter, gridOrientation.gridRotation, GridAxis.AxisXZ, 1.0f, GridMode.Regular);
                }
            }
        }

        public static void RenderGrid(SceneView sceneView)
        {
        ...
        public static void UpdateGridOrientation(Camera camera)
        {
            ...
            var cameraPosition = camera.cameraToWorldMatrix.MultiplyPoint(MathConstants.zeroVector3);
            var cameraForward = camera.cameraToWorldMatrix.MultiplyVector(MathConstants.forwardVector3);

            gridOrientation = new GridOrientation();
            gridOrientation.gridCamera = camera;
            gridOrientation.gridCameraPosition = cameraPosition;
            ...
            if (camera.orthographic)
            {
                ...
                Vector3 dots = new Vector3(
                        Mathf.Clamp01(Mathf.Abs(Vector3.Dot(cameraForward, gridOrientation.gridRotation * MathConstants.rightVector3)) - 0.6f),
                        Mathf.Clamp01(Mathf.Abs(Vector3.Dot(cameraForward, gridOrientation.gridRotation * MathConstants.upVector3)) - 0.3f),
                        Mathf.Clamp01(Mathf.Abs(Vector3.Dot(cameraForward, gridOrientation.gridRotation * MathConstants.forwardVector3)) - 0.6f)
                    ).normalized;
            ...
            var snapScale = Quaternion.Euler(euler) *
            ...
            snapScale.x = Mathf.Abs(snapScale.x);
            snapScale.y = Mathf.Abs(snapScale.y);
            snapScale.z = Mathf.Abs(snapScale.z);

            gridOrientation.gridSnapScale = snapScale;
        }
