...
namespace RealtimeCSG
{
	internal abstract class GeneratorBase : ScriptableObject//, IBrushGenerator
	{
		...
		protected void PaintSnapVisualisation()
		{
			...
			{
				PaintUtility.DrawDottedLines(visualSnappedGrid.ToArray(), ColorSettings.GeneratorGuideline);
			}
			...
			if (visualSnappedBrush != null)
			{
					...
					&& visualSnappedBrush.ChildData.ModelTransform)
				{
					...
					var brushTransformation = visualSnappedBrush.compareTransformation.localToWorldMatrix;

					CSGRenderer.DrawOutlines(visualSnappedBrush.brushNodeID, brushTransformation, color, color, color, color, GUIConstants.oldThickLineScale);
				}
			}
		...
		protected bool GenerateBrushObjects(int brushObjectCount, bool inGridSpace = true)
		{
			...
			return generatedBrushes != null 
				&& generatedBrushes.Length > 0 
				&& generatedBrushes.Length == brushObjectCount;
		}
		...
		private CSGBrush _selectedBrush;
		public virtual void HandleEvents(SceneView sceneView, Rect sceneRect)
		{
            ...
            switch (editMode)
			{
				...
				case EditMode.CreateShape:	
					HandleCreateShapeEvents(sceneView, sceneRect); 
					break;
