...
namespace Chisel.Editors
{
    internal sealed partial class ChiselMaterialBrowserWindow : EditorWindow
    {
        ...
        private static List<CSGModel> m_Models = new List<CSGModel>();
        ...
        private void OnGUI()
        {
            if(m_Tiles.Count > 0 && Event.current.type == EventType.KeyDown)
            {
                var navigate = false;
                if(Event.current.keyCode == KeyCode.RightArrow)
                {
                    lastSelectedMaterialIndex++;
                    navigate = true;
                }
                ...
                {
                    lastSelectedMaterialIndex -= (int)(m_ScrollViewRect.width / tileSize);
                    navigate = true;
                }
                ...
                if (navigate)
                {
                    lastSelectedMaterialIndex = Mathf.Clamp(lastSelectedMaterialIndex, 0, m_Tiles.Count - 1);
                    previewEditor = Editor.CreateEditor(previewMaterial = GetPreviewMaterial(lastSelectedMaterialIndex));
                                    ...
                                    GUI.BeginGroup(m_PropsAreaRect, propsSectionBG);
                                    {
                                        ...
                                        //if (position.height > 520)
                                        {
                                            ...
                                            GUI.BeginGroup(m_PropsAreaRect);
                                            {
                                                ...
                                                var surfaceType = (SurfaceConfigurator.SurfaceType)EditorGUILayout.EnumPopup(m_Tiles[lastSelectedMaterialIndex].surfaceConfig.SurfaceType);
                                                if (surfaceType != m_Tiles[lastSelectedMaterialIndex].surfaceConfig.SurfaceType)
                                                {
                                                    m_Tiles[lastSelectedMaterialIndex].surfaceConfig.SurfaceType = surfaceType;
                                                    EditorUtility.SetDirty(m_Tiles[lastSelectedMaterialIndex].surfaceDb);
                                                }
                    ...
                    for (int x = 0; x < numColumns; x++)
                    {
                        ...
                        if (current[idx].CheckVisible(yOffset, tileSize, tileScrollPos, m_ScrollViewRect.height))
                        {
                            ...
                            if (GUI.Button(m_TileContentRect, m_TileContentText, tileButtonStyle))
                            {
                                ...
                                var diff = Time.realtimeSinceStartup - m_Tiles[idx].lastClickTime;
                                if (diff < .3f)
                                {
                                    ApplySelectedMaterial();
                                }
                                m_Tiles[idx].lastClickTime = Time.realtimeSinceStartup;
                            }
        ...
        [Shortcut("Realtime-CSG/Material Browser/Apply Last Selected Material", KeyCode.A, ShortcutModifiers.Shift)]
