...
//			bool?	VertexChannelColor		= (vertexChannels & VertexChannelFlags.Color) == VertexChannelFlags.Color;
            ...
            bool?	GenerateCompound		= (Settings & ModelSettingsFlags.GenerateCompoundCollider) == ModelSettingsFlags.GenerateCompoundCollider;
            ...
            GUILayout.BeginVertical(GUI.skin.box);
            {
                ...
                if (physicsVisible)
                {
                    ...
                    EditorGUI.BeginDisabledGroup(have_no_collider);
                    {
                        ...
                        bool setCompoundValueMixed = have_no_collider ? true : !GenerateCompound.HasValue;
                        bool setCompoundValue = have_no_collider ? false : (GenerateCompound.HasValue ? GenerateCompound.Value : false);
                        {
                            EditorGUI.BeginChangeCheck();
                            {
                                EditorGUI.showMixedValue = setCompoundValueMixed;
                                setCompoundValue = EditorGUILayout.Toggle(GenerateCompoundCollidersContent, setCompoundValue);
                            }
                            if (EditorGUI.EndChangeCheck())
                            {
                                for (int i = 0; i < models.Length; i++)
                                {
                                    if (setCompoundValue)
                                    {
                                        models[i].Settings |= ModelSettingsFlags.GenerateCompoundCollider;
                                    }
                                    else
                                    {
                                        models[i].Settings &= ~ModelSettingsFlags.GenerateCompoundCollider;
                                ...
                                GUI.changed = true;
                                GenerateCompound = setCompoundValue;
                                updateMeshes = true;
                            }
                        }
                        {
