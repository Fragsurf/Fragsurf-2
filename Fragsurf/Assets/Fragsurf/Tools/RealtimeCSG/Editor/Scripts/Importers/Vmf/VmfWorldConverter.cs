...
namespace RealtimeCSG.Importers.Vmf
{
    ...
    public static class VmfWorldConverter
    {
        ...
        public static bool Import(string vmfPath)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < world.Solids.Count; i++)
                {
                    ...
                    var name = GameObjectUtility.GetUniqueNameForSibling(model.transform, "Brush");
                    var gameObject = new GameObject(name);
                    var rcsgBrush = gameObject.AddComponent<CSGBrush>();
                    var tr = model.transform;

                    gameObject.transform.SetParent(tr, true); // Set it's parent to the transform of the Model
                    ...
                    for (int j = solid.Sides.Count; j-- > 0;)
                    {
                        ...
                        Plane clip = new Plane(tr.InverseTransformPoint(new Vector3(side.Plane.P1.X, side.Plane.P1.Z, side.Plane.P1.Y) * inchesInMeters), tr.InverseTransformPoint(new Vector3(side.Plane.P2.X, side.Plane.P2.Z, side.Plane.P2.Y) * inchesInMeters), tr.InverseTransformPoint(new Vector3(side.Plane.P3.X, side.Plane.P3.Z, side.Plane.P3.Y) * inchesInMeters));
                        planes.Add(clip);
                    }
        ...
        public static void Importasdsad(/*CSGModelBase model, */VmfWorld world)
        {
            try
            {
                //model.BeginUpdate();
