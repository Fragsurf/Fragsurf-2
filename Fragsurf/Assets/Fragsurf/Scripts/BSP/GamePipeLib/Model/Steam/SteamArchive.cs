...
namespace GamePipeLib.Model.Steam
{
    ...
    public class SteamArchive : SteamLibrary
    {
        ...
        public SteamArchive(string libraryDirectory) : base(libraryDirectory, true)
        {
            ...
            {
                UnityEngine.Debug.LogWarning($"Error initializing archive options for {libraryDirectory}" + ex);
            }
        }
        ...
        private void InitializeArchiveOptions()
        {
            ...
            if (File.Exists(optionFilePath))
            {
                ...
                foreach (Match match in searchString.Matches(contents))
                {
                    ...
                    {
                        UnityEngine.Debug.LogError($"Exception parsing {optionFilePath} match '{match.Value}' into properties:" + ex);
                    }
                }
            }
        }
        ...
        private void SaveArchiveOptions()
        {
            ...
            {
                UnityEngine.Debug.LogError($"Exception writing archive options for {optionFilePath}:" + ex);
            }
        }
    }
