...
namespace Fragsurf.BSP
{
	...
	public static class VtfProvider
	{
		...
		private const string VtfHeader = "VTF\0";

		///// Methods /////

		public static Texture2D GetImage(Stream stream, int maxWidth = 0, int maxHeight = 0, bool remipmap = true)
		{
			using (var br = new BinaryReader(stream))
			{
				var headerBytes = br.ReadBytes(4);
				var header = Encoding.ASCII.GetString(headerBytes);
				if (!string.Equals(header, VtfHeader)) throw new Exception("Invalid VTF header. Expected '" + VtfHeader + "', got '" + header + "'.");
				...
				var reflectivityX = br.ReadSingle();
				var reflectivityY = br.ReadSingle();
				var reflectivityZ = br.ReadSingle();
					...
					for (var face = 0; face < faces; face++)
					{
						for (var slice = 0; slice < depth; slice++)
						{
							for (var mip = mipNum; mip < mipmapCount; mip++)
							{
								...
								var img = LoadImage(br, (uint)wid, (uint)hei, highResImageFormat, remipmap);
		...
		private static void TransformRgba16161616F(byte[] buffer, BinaryReader br, uint width, uint height)
		{
			...
			{
				var hb = (float)Half.ToHalf(bytes, i + b);
				var hg = (float)Half.ToHalf(bytes, i + g);
				var hr = (float)Half.ToHalf(bytes, i + r);
			...
			for (int i = 0, j = 0; i < bytes.Length; i += 8, j += 4)
			{
				...
				var hg = (float)Half.ToHalf(bytes, i + g);
				var hr = (float)Half.ToHalf(bytes, i + r);
				var ha = (float)Half.ToHalf(bytes, i + a);
