...
namespace Fragsurf.Shared.Entity
{
    public class Equippable : NetEntity, IInteractable, IDamageable
    {
        ...
        {
            _itemName = itemName;
            var data = DataSet.Instance.Equippables.FirstOrDefault(x => string.Equals(x.Name, itemName, StringComparison.OrdinalIgnoreCase));
            if(data == null)
            {
                if (Game.IsHost)
                {
        ...
        public void Damage(DamageInfo dmgInfo)
        {
            if(EquippableGameObject.WorldModel.TryGetComponent(out Rigidbody rb))
            {
                ...
                if (!Game.IsHost)
                {
                    var effect = Game.Pool.Get(DataSet.Instance.GetImpactEffect(SurfaceMaterialType.Metal), 1f);
