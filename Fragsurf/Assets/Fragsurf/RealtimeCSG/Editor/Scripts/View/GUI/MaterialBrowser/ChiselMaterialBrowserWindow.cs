...
namespace Chisel.Editors
{
    internal sealed partial class ChiselMaterialBrowserWindow : EditorWindow
    {
        ...
        private static int m_CurrentPropsTab = 0;
        ...
        private void OnEnable()
        {
            ...
            m_Tiles = new List<ChiselMaterialBrowserTile>();
        ...
        private void OnGUI()
        {
            ...
            //if (activetool != null)
            //{
            //    var surfaceState = new EditModeSurfaceGUI.SurfaceState();
            //    surfaceState.Init(activetool);
            //    if (surfaceState.firstMaterial)
            //    {
            //        previewMaterial = surfaceState.firstMaterial;
            //        previewEditor = Editor.CreateEditor(surfaceState.firstMaterial);
            //        previewEditor.Repaint();
            //        Repaint();
            //        lastSelectedMaterialIndex = FindTileIndex(surfaceState.firstMaterial);
            //    }
            //}
            ...
            if (!EditorApplication.isCompiling && !EditorApplication.isPlaying)
            {
                ...
                m_ToolbarRect.x = 61;
                m_ToolbarRect.y = 4;
                m_ToolbarRect.width = 160;

                var search = GUI.TextField(m_ToolbarRect, m_Search, EditorStyles.miniTextField);
                if (search != m_Search)
                {
                    ChiselMaterialBrowserUtilities.GetMaterials(ref m_Tiles, ref m_Labels, false, "", search);
                    m_Search = search;
                ...
                if (m_ShowPropsArea)
                {
                    ...
                    switch (m_CurrentPropsTab)
                    {
                        ...
                        //        m_PropsAreaRect.width = 260;
                        //        m_PropsAreaRect.height = position.height - 50;
                        //        EditModeSurfaceGUI.OnGUIContents(m_PropsAreaRect, EditModeManager.ActiveTool as EditModeSurface, false);
                        //    }
        ...
        private static Material GetPreviewMaterial(int index)
        {
            ...
            if (m == null) return Resources.Load<Material>("Default Material");
