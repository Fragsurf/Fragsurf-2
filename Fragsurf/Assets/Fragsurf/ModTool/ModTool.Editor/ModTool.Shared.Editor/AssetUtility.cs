...
namespace ModTool.Shared.Editor
{
    ...
    public class AssetUtility
    {
        ...
        {
            var assets = AssetDatabase.FindAssets("ModTool-Candy");
            foreach (var asset in assets)
            {
                var path = AssetDatabase.GUIDToAssetPath(asset);
                if (Path.GetFileName(path) == "ModTool-Candy.txt")
                {
                    return Path.GetDirectoryName(path);
                }
            }
            throw new Exception("ModTool directory not found");
        }
        ...
        public static List<string> GetAssets(string filter)
        {
            ...
            foreach (string guid in assetGuids)
            {
                ...
                if (assetPath.StartsWith("Assets/Fragsurf"))
                    continue;
