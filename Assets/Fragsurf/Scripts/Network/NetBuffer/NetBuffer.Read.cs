...
namespace Fragsurf.Network
{
	...
	public partial class NetBuffer
	{
		...
		{
			UnityEngine.Debug.Assert(m_bitLength - m_readPosition >= 1, c_readOverflowError);
		...
		{
			UnityEngine.Debug.Assert(m_bitLength - m_readPosition >= 8, c_readOverflowError);
		...
		{
			UnityEngine.Debug.Assert(numberOfBits > 0 && numberOfBits <= 8, "ReadByte(bits) can only read between 1 and 8 bits");
		...
		{
			UnityEngine.Debug.Assert(m_bitLength - m_readPosition + 7 >= (numberOfBytes * 8), c_readOverflowError);
		...
		{
			UnityEngine.Debug.Assert(m_bitLength - m_readPosition + 7 >= (numberOfBytes * 8), c_readOverflowError);
			UnityEngine.Debug.Assert(offset + numberOfBytes <= into.Length);
		...
		{
			UnityEngine.Debug.Assert(m_bitLength - m_readPosition >= numberOfBits, c_readOverflowError);
			UnityEngine.Debug.Assert(offset + BytesToHoldBits(numberOfBits) <= into.Length);
		...
		{
			UnityEngine.Debug.Assert(m_bitLength - m_readPosition >= 16, c_readOverflowError);
		...
		{
			UnityEngine.Debug.Assert(m_bitLength - m_readPosition >= 32, c_readOverflowError);
		...
		{
			UnityEngine.Debug.Assert(numberOfBits > 0 && numberOfBits <= 32, "ReadInt32(bits) can only read between 1 and 32 bits");
			UnityEngine.Debug.Assert(m_bitLength - m_readPosition >= numberOfBits, c_readOverflowError);
		...
		{
			UnityEngine.Debug.Assert(numberOfBits > 0 && numberOfBits <= 32, "ReadUInt32(bits) can only read between 1 and 32 bits");
		...
		{
			UnityEngine.Debug.Assert(m_bitLength - m_readPosition >= 64, c_readOverflowError);
		...
		{
			UnityEngine.Debug.Assert(numberOfBits > 0 && numberOfBits <= 64, "ReadUInt64(bits) can only read between 1 and 64 bits");
			UnityEngine.Debug.Assert(m_bitLength - m_readPosition >= numberOfBits, c_readOverflowError);
		...
		{
			UnityEngine.Debug.Assert(((numberOfBits > 0) && (numberOfBits <= 64)), "ReadInt64(bits) can only read between 1 and 64 bits");
		...
		public int ReadRangedInteger(int min, int max)
		{
	        ...
	        public long ReadRangedInteger(long min, long max)
	        {
	            ...
	            int numBits = BitsToHoldUInt64(range);
		...
		public string ReadString()
		{
			...
			if ((ulong)(m_bitLength - m_readPosition) < ((ulong)byteLen * 8))
			{
				...
				throw new Exception(c_readOverflowError);
#else
