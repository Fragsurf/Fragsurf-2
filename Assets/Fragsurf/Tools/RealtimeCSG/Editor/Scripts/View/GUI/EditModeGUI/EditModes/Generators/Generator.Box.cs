...
namespace RealtimeCSG
{
    ...
    internal sealed class GeneratorBox : GeneratorExtrudedBase, IGenerator
	{
		...
		void PaintShape(SceneView sceneView, int id)
		{
			...
			var realVertices = settings.GetVertices(buildPlane, worldPosition, gridTangent, gridBinormal, out bool isValid);
			...
			{
				var wireframeColor = ColorSettings.BoxBuilderColor;
				...
				{
					PaintUtility.DrawLine(realVertices[i - 1], realVertices[i], lineScale, wireframeColor);
					//PaintUtility.DrawDottedLine(realVertices[i - 1], realVertices[i], wireframeColor, 4.0f);
				}

				PaintUtility.DrawLine(realVertices[realVertices.Length - 1], realVertices[0], lineScale, wireframeColor);
		...
		protected override void HandleCreateShapeEvents(SceneView sceneView, Rect sceneRect)
		{
				...
				camera.pixelRect.Contains(Event.current.mousePosition))
			{
				...
				if (editMode == EditMode.CreatePlane)
				{
					...
					else if (settings.vertices.Length == 2)
					{
						onLastPoint = true;
					}
				} 
